...
import { Zap, Info, ShieldAlert, Download } from 'lucide-react';
    ...
    <header className="w-full max-w-7xl mx-auto px-6 pt-8 pb-4 flex items-center justify-between relative z-10 flex-shrink-0">
      ...
      <div className="flex items-center gap-2">
        {isInstallable && (
          <button 
            onClick={install}
            className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/5 hover:bg-white/10 text-xs font-bold text-violet-400 border border-violet-500/20 transition-all"
          >
            ...
            <TooltipContent side="bottom" className="max-w-xs bg-zinc-900 border-zinc-800 text-zinc-300 p-3 shadow-2xl">
              <div className="space-y-2">
                ...
                <p className="text-xs leading-relaxed text-zinc-400">
                  Direct SDXL Base 1.0 pipeline. Global request limits apply across all active sessions to maintain latency.
                </p>
              </div>
            </TooltipContent>
          </Tooltip>
        ...
        <footer className="w-full max-w-7xl mx-auto px-6 py-6 text-center relative z-10 flex-shrink-0">
          <div className="flex flex-col items-center gap-1">
            ...
            <div className="flex items-center gap-2 mt-2">
              ...
              <span className="text-[9px] text-zinc-500 font-bold uppercase tracking-tighter">Engine Online</span>
            </div>
          </div>
        </footer>
