...
export function HistoryDrawer({ children }: { children: React.ReactNode }) {
  ...
  const handleShare = async (img: SavedImage) => {
    try {
            ...
            <div className="max-w-2xl mx-auto">
              ...
              {images.length === 0 ? (
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                  {images.map((img) => (
                    <div key={img.id} className="group relative aspect-square rounded-2xl overflow-hidden bg-zinc-900 border border-white/5 shadow-lg">
                      <img 
                        src={img.url} 
                        alt={img.prompt}
                      ...
                      <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-end p-2">
                        ...
                        <div className="flex gap-1">
                          <Button size="icon" variant="secondary" className="h-7 w-7 rounded-lg glass" onClick={() => handleShare(img)}>
                            <Share2 className="w-3.5 h-3.5" />
                          </Button>
                          <Button size="icon" variant="secondary" className="h-7 w-7 rounded-lg glass" onClick={() => handleDownload(img)}>
                            <Download className="w-3.5 h-3.5" />
                          </Button>
                          <Button size="icon" variant="destructive" className="h-7 w-7 rounded-lg bg-rose-500/20 text-rose-500 border-none hover:bg-rose-500 hover:text-white" onClick={() => handleDelete(img.id)}>
